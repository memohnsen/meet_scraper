    ...
    return await page.evaluate(() => {
        ...
        return rows.map(row => {
            ...
            return {
                ...
                bestSnatch: getNumber(cells[11]),
                bestCleanJerk: getNumber(cells[12]),
...
function saveData(data: EventData[]) {
    ...
    const formattedResults = allResults
        .map(result => `  { lifter: "${result.lifter}", bodyWeight: ${result.bodyWeight}, bestSnatch: ${result.bestSnatch}, bestCleanJerk: ${result.bestCleanJerk}, total: ${result.total} }`)
        .join(',\n');
